...
checkAuth('log-roles'); // cek apakah sudah login dan punya akses ke menu ini
...
if (isset($_POST['filter'])) {
    ...
    if (!empty($tanggal)) {
        ...
        $sql = "SELECT * FROM `tlog_roles`
                WHERE DATE(created_at) = '$tanggal'
                ORDER BY created_at DESC;";
    } else {
        // Kalau tanggal kosong, bisa kasih hasil kosong atau fallback semua
        $sql = "SELECT * FROM tlog_roles WHERE 1=0"; // tidak tampilkan apa2
    }
...
<head>
    ...
    <title>iSubcont - Logging Roles</title>
