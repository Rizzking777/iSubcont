...
SELECT 
    ...
    MIN(q.lot) as lot_min,
    MAX(q.lot) as lot_max,
    COUNT(DISTINCT q.lot) as total_lot,

    k.komponen,

    GROUP_CONCAT(DISTINCT q.size ORDER BY q.size SEPARATOR ', ') as sizes,

    SUM(q.qty_unique) as total_qty,

    MAX(q.status) as status,
    MAX(q.last_gate) as last_gate,
    MAX(q.transac_by) as transac_by
...
JOIN (
    SELECT 
        ...
        style,
        ncvs,
        GROUP_CONCAT(DISTINCT nm_komponen_in SEPARATOR ', ') as komponen
    FROM tbl_transaksi
    WHERE DATE(created_at) = ?
    GROUP BY 
        job_order, bucket, po_code, po_item, model, style, ncvs
...
ON 
    q.job_order = k.job_order
    AND q.bucket = k.bucket
    AND q.po_code = k.po_code
    AND q.po_item = k.po_item
    AND q.model = k.model
    AND q.style = k.style
    AND q.ncvs = k.ncvs

GROUP BY 
    q.job_order, q.bucket, q.po_code, q.po_item, q.model, q.style, q.ncvs

ORDER BY q.job_order ASC
";
          ...
          <div class="card">
            <div class="card-header text-black">
              <div class="d-flex justify-content-between align-items-center w-100">
                <a href="trans-barcode-create.php" target="_blank" class="btn btn-primary">
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        if ($row['total_lot'] == 1) {
                          ...
                          echo $row['lot_min'] . " - " . $row['lot_max'];
                        }
                        ?>
                      </td>
                      <td><?= htmlspecialchars($row["komponen"]); ?></td>
                      <td><?= htmlspecialchars($row["total_qty"]); ?></td>
                      <td><?= htmlspecialchars($row["last_gate"]); ?></td>
                      <!-- Status -->
                      <td>
                        <?php
                      ...
                      <td><?= htmlspecialchars($row["transac_by"]); ?></td>
